...
def show_box(box, ax, label,color):
    ...
    ax.add_patch(plt.Rectangle((x0, y0), w, h, edgecolor=color, facecolor=(0,0,0,0), lw=2)) #try and change from all green
    ax.text(x0, y0, label)
...
if __name__ == "__main__":
    ...
    for j in range(1,6):
        num= str(j)
        image_path= 'seqtestimages/image_'+num+'.png'
        ...
        text_prompt = "Hedge. Path. Flowers. Grass. Pole. Other" #Up to 15 prompts for now Greenhouse. Net. Tractor. Bush. Flowers. Grass. Sky. Hill. Car. 
        colours= ['red','orange','yellow','green','blue','purple','white','pink','olive','cyan','brown','gray','coral','lavender','beige']
        output_dir = "seq_hq_grounded_sam_result"
        box_threshold = 0.25
        text_threshold = 0.25
        device = "cpu"
