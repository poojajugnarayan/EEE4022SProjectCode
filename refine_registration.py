...
def make_posegraph_for_refined_scene(ply_file_names, config):
    temp= config["path_dataset"]+ config["template_global_posegraph_optimized"]+"pose.json"
    pose_graph = o3d.io.read_pose_graph(temp)
    ...
    temp=config["path_dataset"]+ config["template_refined_posegraph"]+"pose.json"
    o3d.io.write_pose_graph(temp,pose_graph_new)
...
def run(config):
    ...
    #     join(config["path_dataset"], config["template_fragment_pointcloud"]), ".ply")
    ply_file_names=[]
