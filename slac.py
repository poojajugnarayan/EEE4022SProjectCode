...
def run(config):
    ...
    ply_file_names=[]
    frags=(int)(config["n_frames"]/config["n_frames_per_fragment"])
    for i in range(0, frags):
        pcd_name = config["path_dataset"]+config["template_fragment_pointcloud"]+"frag"+str(i)+".ply"
        ply_file_names.append(pcd_name)
    ...
    temp=config["path_dataset"]+config["template_optimized_posegraph_slac"]+"pose.json"
    # Write updated pose graph.
    o3d.io.write_pose_graph(temp, pose_graph_updated)

    # Write trajectory for slac-integrate stage.
    fragment_folder = path_dataset+ config["template_fragment_posegraph_optimized"]
    params = []
    for i in range(len(pose_graph_updated.nodes)):
        temp=fragment_folder+ "frag"+str(i)+".json"
        fragment_pose_graph = o3d.io.read_pose_graph(temp)
    ...
    temp= config["path_dataset"]+config["template_global_traj"]+"op.log"
    o3d.io.write_pinhole_camera_trajectory(temp, trajectory)
